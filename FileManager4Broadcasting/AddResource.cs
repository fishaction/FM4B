...
namespace FileManager4Broadcasting
{
    ...
    static class AddResource
    {
        ...
        static public FilesAttribute GetFile(string projectName, string fileName)
        {
            string saveLocation = Properties.Settings.Default.saveLocation + @"\FM4B\プロジェクト\" + projectName;
            if (!File.Exists(saveLocation + @"\files.json"))
                return null;
        ...
        static public void ModifyJsonFile(int n,string path, string projectName, string d, ResourceType type, string[] _tags, DateTime cDate, bool _isLinked)
        {
            string saveLocation = Properties.Settings.Default.saveLocation + @"\FM4B\プロジェクト\" + projectName;
            ...
            files[n].FilePath = path;
            files[n].FileName = Path.GetFileName(path);
            files[n].Description = d;
            files[n].ResourceType = type.ToString();
            files[n].Tags = _tags;
            files[n].CreatedDate = cDate;
            files[n].IsLinked = _isLinked;
            foreach (FilesAttribute fa in files)
            {
